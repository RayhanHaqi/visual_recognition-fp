...
def predict_image_tiled(
    model, image_path: Path, tile_size: int, device, shifts: int, stride: int
) -> np.ndarray:
    ...
    return aggregate_tile_predictions(preds, windows, w, h, mode="sum")
...
def main():
    ...
    for img_id in tqdm(sample[id_col].astype(str), desc="inference"):
        ...
        path = path_by_id.get(stem)
        if path is None:
            path = path_by_id.get(img_id)
        if path is None:
            raise FileNotFoundError(f"Test image not found for id={img_id}")
        pred = predict_image_tiled(model, path, tile_size, device, args.shifts, stride)
